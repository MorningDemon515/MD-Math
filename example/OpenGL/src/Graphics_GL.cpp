...
void Draw_OpenGL(){
	...
	MATRIX T = TranslationMatrix(0.0f,0.0f,-3.0f);
	MATRIX view = ViewMatrixRH(eye, target, up);
	MATRIX projection = (PerspectiveMatrixRH(AngularToRadian(45.0f),
		(float)WINDOW_WIDTH / (float)WINDOW_HEIGHT,
		0.1f,
		100.0f));
	
	MATRIX Rx, Ry;
	Rx = RotationMatrix(3.14f / 4.0f , 'X');
	
	static float y = 0.0f;
    Ry = RotationMatrix(y, 'Y');
	y += TimeDelta();
	
	if( y >= 6.28f )
		y = 0.0f;
	
	MATRIX model = T * Rx * Ry;
